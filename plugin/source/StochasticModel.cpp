...
{
    // 1. Get atomic values
    float currentGlobalDensity = globalDensity_.load(std::memory_order_relaxed);
    double currentSampleRate = sampleRate_.load(std::memory_order_relaxed);
    ...
    if (currentGlobalDensity <= 0.0f || currentSampleRate <= 0.0)
    {
    ...
    double averageSamplesPerGrain = currentSampleRate / static_cast<double>(currentGlobalDensity);
